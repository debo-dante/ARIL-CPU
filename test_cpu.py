...
aplnet = torch.load('weights/net1111_Train100.0Test88.129Train99.910Test95.683.pkl', map_location=torch.device('cpu'))
aplnet = aplnet.eval()  # Removed .cuda() call
...
for i, (samples, labels) in enumerate(test_data_loader):
    with torch.no_grad():
        ...
        # sio.savemat('vis/fig_tsne/out_act_conf.mat', {'out_max': act1.view(act1.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_loc_conf.mat', {'out_max': loc1.view(loc1.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_maxpool.mat', {'out_max': x.view(x.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_c1.mat', {'out_max': c1.view(c1.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_c2.mat', {'out_max': c2.view(c2.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_c3.mat', {'out_max': c3.view(c3.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_c4.mat', {'out_max': c4.view(c4.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_fc_act.mat', {'out_max': act.view(act.shape[0], -1).numpy()})
        # sio.savemat('vis/fig_tsne/out_fc_loc.mat', {'out_max': loc.view(loc.shape[0], -1).numpy()})

        prediction = predict_label_act.data.max(1)[1]
        sio.savemat('vis/actResult.mat',{'act_prediction':prediction.numpy()})
        correct_test_act += prediction.eq(labelsV_act.data.long()).sum()
        print(correct_test_act.numpy()/num_test_instances)

        prediction = predict_label_loc.data.max(1)[1]
        sio.savemat('vis/locResult.mat', {'loc_prediction': prediction.numpy()})
        correct_test_loc += prediction.eq(labelsV_loc.data.long()).sum()
        print(correct_test_loc.numpy() / num_test_instances)
